...
from scripts.tool import GradeSystem, predict
...
if not os.path.isdir(feedback_dir):
    current_app.logger.info("creating feedback data directory: {}".format(feedback_dir))
...
def log_to_pickle(input_text, output_dict):
    ...
    filepath = "{dir}{name}.pkl".format(dir=log_dir, name=filename)
...
def record_feedback(feedback_json):
    timestamp = datetime.now()
    filename = timestamp.strftime("%Y%m%d_%H%M%S")
    filepath = "{dir}{name}.json".format(dir=feedback_dir, name=filename)
...
def get_score(input_text):
    ...
    inputs = GradeSystem.Feature(
        ngram=ngram, pos_ngram=pos_ngram, grmitem=grm, word_difficulty=diff, stats=stats
    ).concat()
    ...
    try:
        ...
        if request.method == "POST":
            try:
                ...
                error_responce = {"api_status_code": 601, "message": "JSONSchemaError"}
            ...
            try:
                ...
                current_app.logger.critical("exception during outputing log to pickle")
                current_app.logger.exception(e)
            sum_of_rate = sum(output_dict["word_diff"])

            # by kawamoto 2022/06/07
            # g_contents = [item.decode('utf8') for item in output_dict['grmitem']]
            g_contents = output_dict["grmitem"]
            ...
            result_json = {
                ...
                "vocabulary_rates": [
                    {"level": level_name, "rate": level_rate}
                    for level_name, level_rate in zip(
                        ["A1", "A2", "B1", "機能語"],
                        [
                            round(num / sum_of_rate * 100, 1)
                            for num in output_dict["word_diff"]
                        ],
                    )
                ],
                "used_grammer_contents": [
                    {"grammer_type": gc_name, "frequency": 1} for gc_name in g_contents
                ],
            }
            ...
            try:
                ...
                current_app.logger.critical("exception during outputing log to pickle")
                current_app.logger.exception(e)
            feedback_responce = {"api_status_code": 200, "message": "Success"}
            try:
        ...
        current_app.logger.critical(
            "unhandled exception was raised. " "check traceback"
        )
        current_app.logger.exception(e)
...
def index():
    ...
    if input_text:
        ...
        #    flask.flash("非ASCII文字の入力は現在対応中です。" "非ASCII文字を無視してスコアを計算しました。")
        ...
        chart_datas = [
            {
                "data": [
                    # by kawamoto
                    round(num / sum_of_rate * 100, 1) if sum_of_rate else 0
                    for num in output_dict["word_diff"]
                ],
                "backgroundColor": [
                    "rgba(255, 0, 0, 0.2)",
        ...
        chart_labels = ["A1", "A2", "B1", "B2"]  # , "機能語"] by kawamoto
        chart_data = {"datasets": chart_datas, "labels": chart_labels}
        ...
        # by kawamoto 2022/06/07
        # g_contents = [(item[0].decode("utf8"), item[1]) for item in output_dict["grmitem"]]
        g_contents = [(item[0], item[1]) for item in output_dict["grmitem"]]
